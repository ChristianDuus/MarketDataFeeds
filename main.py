...
def update_google_sheet(symbol, bids, asks):
    try:
        ...
        if current_time - last_update_times[symbol] < update_freq:
            print(f"Skipping update for {symbol}. Time since last update: {current_time - last_update_times[symbol]:.2f} seconds")
            return
        ...
        print(f"Google Sheet updated for {symbol} at {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(last_update_times[symbol]))}")
